...
class Model_Finder:
    ...
    def get_best_params_for_random_forest(self,train_x,train_y):
        ...
        try:
            ...
            self.grid = GridSearchCV(estimators=self.clf, param_grid = self.param_grid,cv=5,verbose=3)
    ...
    def get_best_params_for_knn(self,train_x,train_y):
        ...
        try: 
            ...
            self.logger_object.log(self.file_object,'KNN best params: '+str(self.grid.best_params_)+'. Exited the get_best_params_for_knn method of the Model_Finder class')
            return self.knn
        except Exception as e:
            self.logger_object.log(self.file_object,'Exception occured in get_best_params_for_knn method fo the Model_Finder class. Exception message: '+str(e))
            self.logger_object.log(self.file_object,'knn Parameter tuning failed. Exited the get_best_params_for_knn method of the Model_Finder class.')
            raise Exception()
    ...
    def get_best_model(self, train_x,train_y,test_x,test_y):
        ...
        try:
            ...
            self.prediction_knn = self.knn.predict(test_x) # Prediction using the KNN model

            if len(test_y.unique()) == 1: # if there is only one label in y, then roc_auc_score return error. We will use accuracy in that case
            ...
            if len(test_y.unique()) == 1: # if there is only one label in y, then roc_auc_score returns error. We will use accuracy_score in that case
