...
class dBOperation:
    ...
    def dataBaseConnection(self,DatabaseName):
        """
            ...
            Description: This method creates the database with the given name and if Database already then opens connection to the DB.
            Output: Connection to the DB
            On Failure: Rasie ConnectionError
        """
        try:
            ...
            file = open("Prediction_Logs/DataBaseConnectionLog.txt",'a+')
            self.logger.log(file,"Error while connection to database: %s"%ConnectionError)
            file.close()
    ...
    def createTableDb(self,DatabaseName,column_names):
        ...
        try:
            ...
            for key in column_names.keys():
                ...
                # in try block if the table exists, if yes then add column to the table
                ...
                file = open(file,"Prediction_Logs/DbTableCreateLog.txt",'a+')
                self.logger.log(file, "Closed %s database successfully"%DatabaseName) 
                file.close()
        
        except Exception as e:
            file = open("Prediction_Logs/DbTableCreateLog.txt",'a+')
            self.logger.log(file, "Error hile creating table: %s"%e)
            file.close()
            conn.close()
            file = open("Prediction_Logs/DataBaseConnectionLog.txt",'a+')
            self.logger.log(file, "Closed %s database successfully"%DatabaseName)
    ...
    def selectingDatafromtableintocsv(self,Database):
        ...
        try:
            ...
            # Get the headers ofthe csv file
            headers = [i[0] for i in cursor.description]

            # Make the CSV output ddirectory
