...
class pred_validation:
    ...
    def prediction_validation(self):
        try:
            ...
            self.log_writer.log(self.file_object,'Start of Valdiation on file for prediction!!')
            # extracting values from prediction schema
            LengthOfDateStampInFile, LengthOfTimeStampInFile, column_names, noofcolumns = self.raw_data.valuesFromSchema()
            # getting the regex defined to validate filename
            regex = self.raw_data.manualRegexCreation()
            # validating filename of prediction file
            self.raw_data.valdiationFileNameRaw(regex,LengthOfDateStampInFile,LengthOfTimeStampInFile)
            ...
            # create database with given name, if present open the connection! Create table with columns give in schema
            ...
            # Delete the good ddata foldder after loading files in table
            self.raw_data.deleteExistingGoodDataTrainingFolder()
            self.log_writer.log(self.file_object,"Good_Data folder deleted!!")
            self.log_writer.log(self.file_object,"Moving bad file to Archivve and deleting Bad_Data folder!!")
